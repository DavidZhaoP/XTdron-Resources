                                           ...
                                           const Eigen::Vector3d &end_acc, const Eigen::VectorXd &Time)
{
  ...
  std::cout << "success127=" <<std::endl;

  /* ---------- R matrix ---------- */
  std::cout << "success128=" <<std::endl;
  // Eigen::MatrixXd Rzp = C * A.transpose();
  Eigen::MatrixXd Rzp = C * A.transpose().inverse()* Q * A.inverse()* Ct;
  std::cout << "success128zp=" <<std::endl;
  Eigen::MatrixXd R = C * A.transpose().inverse() * Q * A.inverse() * Ct;
  std::cout << "success131=" <<std::endl;
  Eigen::VectorXd Dxf(2 * seg_num + 4), Dyf(2 * seg_num + 4), Dzf(2 * seg_num + 4);
  std::cout << "success133=" <<std::endl;
  Dxf = Dx1.segment(0, 2 * seg_num + 4);
  Dyf = Dy1.segment(0, 2 * seg_num + 4);
  Dzf = Dz1.segment(0, 2 * seg_num + 4);
  std::cout << "success137=" <<std::endl;
